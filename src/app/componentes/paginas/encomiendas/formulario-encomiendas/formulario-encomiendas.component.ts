...
export class FormularioEncomiendasComponent implements OnInit {
  ...
  sendViaWhatsApp(encomiendaNueva:Encomienda) { 
    ...
    const message = `Usted deposito una encomienda${(encomiendaNueva.viaje?.conductore?.sindicato?.nombre=== undefined) ? '' : " en el sindicato: "+encomiendaNueva.viaje?.conductore?.sindicato?.nombre}.%0A%0AEl codigo privado de la encomienda es:%0A${encomiendaNueva.codigo_encomienda}.%0A%0AUsted puede obtener su recibo digital en el siguiente enlace:%0A${urlRecibo}%0A%0AUsted tambien puede realizar un seguimiento a su encomienda a traves del siguiente enlace:%0A${urlSeguimiento}%0A%0ATenga cuidado de no compartir los enlaces, muchas gracias por su preferencia!`;
